...
| T07 | A | Canary and release decision | In progress | The first approved canary failed closed and restored `v2.28`; a live-state lint gap was fixed test-first; a new exact candidate requires separate approval; no agent self-publishes | `6d494ef` plus pending evidence checkpoint |
...
- 2026-09-04: T07 live-state correction observed red in `tests/test_runtime_worktrees_excluded.py`. `vault_lint.py` now excludes only the explicit host-managed roots `.claude/worktrees/` and `.agents/worktrees/`, while continuing to lint the live host skill-loader trees. The focused contract and full 33-test suite pass, and exact candidate simulation at `6d494ef` passes clean install, upgrade, and rollback. Because this is a new candidate, the earlier approval for `6325c3b` does not authorize installing it.
...
T06 is complete and T07 is in progress. The first canary failed closed and exposed a live-state lint gap, now fixed at `6d494ef`. The release owner must explicitly approve or decline a second canary using the current clean `improve/conversation-harness` branch HEAD. Record the exact commit at approval time. Do not merge, tag, push, publish, or release as part of the canary decision; those remain a later, separate approval gate.
