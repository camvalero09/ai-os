...
| T06 | A | Run clean-install and `v2.28` upgrade simulations | Complete | Exact candidate `3f2b3dd` clean-installs; synthetic personal data and credential permissions survive `v2.28` upgrade; root and skill adapters plus settings rebuild; rollback restores `v2.28` and baseline-generated state; installed System stays clean | `3f2b3dd` plus this evidence checkpoint |
...
- 2026-09-04: the first independent T06 review failed closed on ambient-environment isolation. After that was fixed test-first, a second review identified two remaining logic gaps: a follow-up generator could repair a faulty clean install before inspection, and installed-System cleanliness was not checked after every phase. The installer output is now inspected before any follow-up generation and System cleanliness is part of each phase's pass condition. A fresh final review passed with no security concerns, logic errors, or suggestions.
- 2026-09-04: exact-candidate verification at `3f2b3dd` passed: candidate clean install, `v2.28` upgrade, and rollback all returned true; personal contents and credential permissions were unchanged; root adapters, skill loaders, and settings changed on upgrade and restored on rollback; both installed System checkouts remained clean. The full repository suite passed 32 tests, shared-rule and conversation-fixture checks passed, and maintainer adapters were current.
- 2026-09-04: no live canary was attempted. A read-only prerequisite check found the installed `v2.28` System checkout already contains an untracked self-referential `System/System` symlink created on 2026-09-03. The normal update workflow must stop on any local System change, so T07 is blocked until the release owner separately approves investigating and removing or preserving that symlink.
...
- T06's repeatable release gate passed candidate clean install, `v2.28` upgrade, rollback, environment isolation, and three independent review cycles; the exact implementation checkpoint is `3f2b3dd`.
...
- No live canary, merge, tag, push, or release has occurred. The live installed System has a pre-existing untracked `System/System` symlink that blocks the normal update preflight.

### Exact next action

T06 is complete. T07 requires two separate owner decisions. First, inspect and decide whether to remove the pre-existing untracked self-referential `System/System` symlink so the live-vault update preflight can become clean. Second, only after a clean preflight, explicitly approve or decline installing commit `3f2b3dd` as a live-vault canary. Do not merge, tag, push, publish, or release as part of the canary decision; those remain a later, separate approval gate.
