...
- 2026-09-04: no live canary was attempted. A preflight found an untracked self-referential `System/System` symlink in the installed `v2.28` checkout. After separate owner approval, only that asserted symlink was removed; its target and tracked System files were not changed. The live checkout remains detached at `v2.28` and is ready for a separate canary decision.
...
- No live canary, merge, tag, push, or release has occurred. The pre-existing self-referential symlink was removed after separate owner approval; the live installed System remains at `v2.28`.
