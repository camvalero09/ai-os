...
T05 is complete across four checkpoints (`8b99ef7`, `72bffa1`, `0a3f6c3`, and the T05.4 checkpoint): reserved template entry filenames removed, authoring-only records excluded from vault lint, duplicated shipped style defaults removed, and the remaining contradictions (Effort-log claims, stale statements, duplicate dates, push permission, the redundant Quick reference block) resolved test-first. The next bounded task is T06: run clean-install and `v2.28` upgrade simulations, proving personal data survives, adapters rebuild, and rollback works. Do not install the candidate in the live vault, publish, tag, push, or merge without the later explicit gates.
